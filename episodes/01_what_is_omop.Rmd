...
**CODING_NOTE**: We use `filter` to identify the row we want and `select` to choose the column we want. These are functions from the dplyr package that can make code clearer. This is because we are working with a database connection. If we were working with a local R object we could use base R code: `cdm$concept$concept_name[cdm$concept$concept_id == 9201]` to get the same result.

### A useful function

Finding the humanly readable name for a `concept_id` will be a useful function. We can create a function `get_concept_name()` that takes the 'cdm' object and a `concept_id` as input and returns the `concept_name`.

:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: challenge

Create the function `get_concept_name()` that takes the the 'cdm' object and a `concept_id` as input and returns the `concept_name`.

...
Here is how we can plot blood pressure over time for selected patients using just the measurement table.
...
bp <- cdm$measurement %>%
  ...
  filter(measurement_concept_id %in% c(systolic_id, diastolic_id)) |> 
  mutate(
    bp_type = case_when(
      measurement_concept_id == systolic_id ~ "Systolic",
      measurement_concept_id == diastolic_id ~ "Diastolic"
    )
  )

ggplot(bp, aes(x = measurement_date, y = value_as_number, color = bp_type)) +
  geom_line() +
  geom_point() +

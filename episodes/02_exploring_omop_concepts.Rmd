...
**CODING_NOTE**: We have to use `collect()` to pull the data into R memory to view it. The `arrange()` function orders the rows by concept_id. The `filter(row_number() <= 10)` function filters to the first 10 rows. The `select()` function selects only the specified columns.
...
**CODING_NOTE**: We use `slice_min()` to get the first 10 rows ordered by concept_id.
